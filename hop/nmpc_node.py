...
class NMPCNode(Node):
    ...
    def __init__(self, dt = mc.dt):
        ...
        self.control_history = deque(
            [np.array([-mc.gimbal_offset[0], -mc.gimbal_offset[1], mc.hover_thrust, 0.0]) for i in range(mc.nmpc_delay)],
...
############################# callbacks  ####################################
    ...
    def nmpc(self, msg):
        ...
        if not self.shutdown_reason == ShutdownReason.NONE:     # shutdown time
            ...
            control = np.array([-mc.gimbal_offset[0], -mc.gimbal_offset[1], mc.hover_thrust, 0.0])
